...
class CentralControllerWrapper(gym.Env):
    def __init__(
        ...
        self.observation_space = gym.spaces.Box(
            np.zeros(full_obs_size),
            np.ones(full_obs_size),
            (full_obs_size,),
            np.float32,
        )
    ...
    def enumerate_agent_actions(
        self,
    ):
        agent_actions = [
            np.arange(self.env.action_space[i].n)
            for i in range(len(self.env.action_space))
        ]
        enumerated_actions = np.array(np.meshgrid(*agent_actions)).T.reshape(
            -1, self.num_agents
        )
        action_mapping = {
            int(i): list(action) for i, action in enumerate(enumerated_actions)
        }
