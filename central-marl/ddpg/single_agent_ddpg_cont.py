...
def critic_loss(
    ...
    # NOTE: Look into this.
    target_actions = jax.vmap(policy_network.apply, in_axes=(None, 0))(
        target_policy_params, next_states
    )
    target_state_actions = jnp.concatenate((next_states, target_actions), axis=1)
    target_action_values = jax.vmap(critic_network.apply, in_axes=(None, 0))(
        target_critic_params, target_state_actions
    )
    target_action_values = jnp.squeeze(target_action_values)

    online_state_actions = jnp.concatenate((states, actions), axis=1)
    online_action_values = jax.vmap(critic_network.apply, in_axes=(None, 0))(
        critic_params, online_state_actions
    )
...
def policy_loss(
    ...
    online_actions = jax.vmap(policy_network.apply, in_axes=(None, 0))(
