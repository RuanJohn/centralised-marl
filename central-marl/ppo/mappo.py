...
def create_policy_hidden_states(
    ...
    hidden_states = jnp.array(
        [jnp.zeros(layer, dtype=jnp.float32) for layer in policy_recurrent_layer_sizes]
    )
...
def create_critic_hidden_states(
    ...
    hidden_states = jnp.array(
        [jnp.zeros(layer, dtype=jnp.float32) for layer in critic_recurrent_layer_sizes]
    )
...
dummy_joint_obs_data = jnp.zeros(critic_observation_dim, dtype=jnp.float32)
...
policy_params = policy_network.init(
    policy_init_key, dummy_obs_data, policy_hidden_state
)
critic_params = critic_network.init(
    critic_init_key, dummy_joint_obs_data, critic_hidden_state
)
...
buffer_state = create_buffer(
    ...
    joint_observation_dim=critic_observation_dim,
...
def update_policy(system_state: PPOSystemState, advantages, mb_idx):
    ...
    policy_hidden_states_ = jnp.squeeze(
        system_state.train_buffer.policy_hidden_states, axis=2
    )[mb_idx]
    advantages_ = advantages[mb_idx]

    if NORMALISE_ADVANTAGE:
        advantages_ = (advantages_ - jnp.mean(advantages_)) / (
            jnp.std(advantages_) + 1e-5
        )
...
def update_critic(
    ...
    critic_hidden_states_ = jnp.squeeze(
        system_state.train_buffer.critic_hidden_states[:, :, :, 0, :, :], axis=2
    )[mb_idx]
...
while global_step < 200_000:
    ...
    policy_hidden_state = create_policy_hidden_states()
    policy_hidden_state = jnp.broadcast_to(
        policy_hidden_state, (num_agents, *policy_hidden_state.shape)
    )
    ...
    while not team_done:
        ...
        for agent in range(num_agents):
            ...
            new_policy_hidden_state_ = jnp.expand_dims(
                new_policy_hidden_state_[0], axis=0
            )
            ...
            new_policy_hidden_state = new_policy_hidden_state.at[agent].set(
                new_policy_hidden_state_
            )
        ...
        value, new_critic_hidden_state_ = critic_network.apply(
            system_state.network_params.critic_params, joint_obs, critic_hidden_state
        )
        ...
        store_critic_hidden_state = jnp.broadcast_to(
            critic_hidden_state, (num_agents, *critic_hidden_state.shape)
        )
        ...
        if global_step % (HORIZON + 1) == 0:
            ...
            for agent in range(num_agents):
                advantage = rlax.truncated_generalized_advantage_estimation(
                    ...
                    discount_t=(
                        1 - jnp.squeeze(system_state.buffer.dones[:, :, agent])
                    )[:-1]
                    * DISCOUNT_GAMMA,
                    lambda_=GAE_LAMBDA,
                ...
                return_ = (
                    advantage
                    + jnp.squeeze(system_state.buffer.values[:, :, agent])[:-1]
                )
    ...
    if episode % 10 == 0:
        print(
            f"EPISODE: {episode}, GLOBAL_STEP: {global_step}, EPISODE_RETURN: {jnp.round(episode_return, 3)}, SPS: {int(sps)}"
        )
