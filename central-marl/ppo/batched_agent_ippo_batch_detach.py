"""Independent multi-agent JAX PPO. This implements a trick metioned in (https://arxiv.org/pdf/2110.00641.pdf) 
which allows for decoupling the policy updates from the batchsize enabling really large 
horizons without parallel environments."""
...
HORIZON = 104 * C
CLIP_EPSILON = 0.2 
POLICY_LR = 0.005 / jnp.sqrt(C)
CRITIC_LR = 0.005 / jnp.sqrt(C)
DISCOUNT_GAMMA = 0.99 
GAE_LAMBDA = 0.95
NUM_EPOCHS = 2
NUM_MINIBATCHES = 8 * C
...
policy_optimiser = optax.chain(
      ...
      optax.adam(learning_rate = POLICY_LR, eps = ADAM_EPS, b1=0.9**(1./C), b2=0.999**(1./C)),
    )
...
def policy_loss(
    ...
    proximal_logits = policy_network.apply(proximal_polcy_params, states)
    proximal_dist = distrax.Categorical(logits=proximal_logits)
    proximal_log_probs = proximal_dist.log_prob(value=actions)
    multiplier = jnp.exp(proximal_log_probs - old_log_probs)

    ratio = jnp.exp(new_log_probs - proximal_log_probs)

    # Policy loss
    loss_term_1 = -advantages * ratio
    loss_term_2 = -advantages * jnp.clip(ratio, 1 - CLIP_EPSILON, 1 + CLIP_EPSILON)
    loss = jnp.maximum(loss_term_1, loss_term_2)
    loss = (multiplier * loss).mean() 
...
batched_policy_loss = jax.vmap(policy_loss, in_axes=(None, None, 1, 1, 1, 1, 1))
def full_policy_loss(
    policy_params, 
    proximal_polcy_params,
    states, 
    actions, 
    ...
    return jnp.mean(batched_policy_loss(
        policy_params, 
        proximal_polcy_params,
        states, 
        actions, 
...
def update_policy(system_state: PPOSystemState, advantages, mb_idx): 
    ...
    proximal_policy_params = system_state.network_params.target_policy_params

    grads = jax.grad(full_policy_loss)(
        policy_params, 
        proximal_policy_params,
        states_, 
        actions_, 
        old_log_probs_, 
        advantages_,
    ...
    # Compute proximal policy params as a exponential moving average of the policy params
    # This is used to compute the ratio in the policy loss
    w = system_state.network_params.w
    w_new = 1. + BETA_PROX * w
    
    system_state.network_params.target_policy_params = jax.tree_util.tree_map(
        lambda p, e: (1. / w_new) * e + BETA_PROX * (w / w_new) * p, 
        system_state.network_params.policy_params, 
        system_state.network_params.target_policy_params)

    system_state.network_params.w = w_new
