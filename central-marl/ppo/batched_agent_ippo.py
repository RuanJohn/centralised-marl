...
def full_policy_loss(
    policy_params, states, actions, old_log_probs, advantages, entropies_
):
    return jnp.mean(
        batched_policy_loss(
            policy_params, states, actions, old_log_probs, advantages, entropies_
...
def update_policy(system_state: PPOSystemState, advantages, mb_idx):
    ...
    old_log_probs_ = jnp.squeeze(system_state.buffer.log_probs[:, :, :])[
        jnp.array(mb_idx)
    ]
...
while global_step < 200_000:
    ...
    while not team_done:
        ...
        actor_keys = keys[1:]
        _, step_joint_action, act_log_probs, act_entropies = batched_choose_action(
            logits, actor_keys
        )
        act_joint_action = jnp.expand_dims(step_joint_action, axis=1)
        act_values = jnp.squeeze(
            batched_critic_apply(system_state.network_params.critic_params, obs)
        )
