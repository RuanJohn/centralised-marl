...
def reshape_logits(logits):
    reshaped_logits = jnp.array(
        jnp.split(logits, indices_or_sections=action_chunk_dims[1:], axis=0)
    )
...
while global_step < 200_000:
    ...
    while not done:
        ...
        value = jnp.squeeze(
            critic_network.apply(system_state.network_params.critic_params, obs)
        )
    ...
    if episode % 10 == 0:
        print(
            f"EPISODE: {episode}, GLOBAL_STEP: {global_step}, EPISODE_RETURN: {episode_return}, SPS: {sps}"
        )
