...
from utils.extra_metrics import (
    diff_reward_step,
    create_visitation_grids,
)
...
def policy_loss(policy_params, states, actions, old_log_probs, advantages, entropies_):
...
def critic_loss(critic_params, states, returns):
...
episode = 0
log_data = {}
while global_step < 20_000:
    team_done = False
    obs = env.reset()
    obs = jnp.array(obs, dtype=jnp.float32)
    episode_return = 0
    agent_diff_returns = {i: 0 for i in range(num_agents)}
    episode_step = 0
    ...
    while not team_done:
        ...
        for agent in range(num_agents):
            ...
            logits = policy_network.apply(
                system_state.network_params.policy_params, obs[agent]
            )
            ...
            value = jnp.squeeze(
                critic_network.apply(
                    system_state.network_params.critic_params, obs[agent]
                )
            )
        ...
        diff_rewards = diff_reward_step(env, step_joint_action, AGENT_NOOP_IDXS)
        for agent in range(num_agents):
            agent_diff_returns[agent] += diff_rewards[agent]
    ...
    if episode % 10 == 0:
        ...
        diff_results = [
            f"AGENT_{i}: {jnp.round(agent_diff_returns[i], 3)}"
