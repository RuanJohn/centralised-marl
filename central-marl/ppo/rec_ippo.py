...
critic_params = critic_network.init(
    critic_init_key, dummy_obs_data, critic_hidden_state
)
...
def update_critic(
    ...
    critic_hidden_states_ = jnp.squeeze(
        system_state.train_buffer.critic_hidden_states, axis=2
    )[mb_idx]
...
while global_step < 200_000:
    ...
    critic_hidden_state = create_critic_hidden_states()
    critic_hidden_state = jnp.broadcast_to(
    ...
    while not team_done:
        ...
        for agent in range(num_agents):
            ...
            new_critic_hidden_state_ = jnp.expand_dims(
                new_critic_hidden_state_[0], axis=0
            )
            ...
            new_critic_hidden_state = new_critic_hidden_state.at[agent].set(
                new_critic_hidden_state_
            )
