...
(
    ...
    env_create_key,
    env_reset_key,
) = jax.random.split(MASTER_PRNGKEY, 6)
...
def rollout(
    rng_input, policy_params, critic_params, env_params, steps_in_episode, obs, state
):
    ...
    if rollouts % 10 == 0:
        ...
        while not done:
            logits = policy_network.apply(
                system_state.network_params.policy_params, eval_obs
            )
            eval_act_key, eval_sample_key, eval_step_key = jax.random.split(
                eval_act_key, 3
            )
            _, action, logprob, entropy = choose_action(logits, eval_sample_key)
            n_eval_obs, n_eval_state, reward, done, _ = env.step(
                eval_step_key, eval_state, action, eval_env_params
            )
            returns += reward
