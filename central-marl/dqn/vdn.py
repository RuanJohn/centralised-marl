...
def select_q_values(q_value, action, target_q_value):
    ...
    chex.assert_type([q_value, action, target_q_value], [float, int, float])
...
def dqn_loss(
    ...
    q_out = jnp.empty(
        (BATCH_SIZE, SEQUENCE_LENGTH, num_agents, num_actions), dtype=jnp.float32
    )
    q_next_out = jnp.empty(
        (BATCH_SIZE, SEQUENCE_LENGTH, num_agents, num_actions), dtype=jnp.float32
    )
    selector_q_out = jnp.empty(
        (BATCH_SIZE, SEQUENCE_LENGTH, num_agents, num_actions), dtype=jnp.float32
    )
    ...
    for agent_idx in range(num_agents):
        agent_policy_hidden_states = jnp.zeros_like(
            policy_hidden_states[:, 0, agent_idx, :, :]
        )
    ...
    q_out, q_next_out = batched_select_q_values(
        q_out, actions, q_next_out, selector_q_out
    )
...
while global_step < 500_000:
    ...
    if episode % 1 == 0:
        print(
            f"EPISODE: {episode}, GLOBAL_STEP: {global_step}, EPISODE_RETURN: {episode_return}, EPSILON: {jnp.round(EPSILON, 2)}"
        )
