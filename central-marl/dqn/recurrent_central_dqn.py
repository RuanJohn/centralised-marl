...
def create_epsilon_schedule(
    ...
    def linear_epsilon_schedule(step):
        ...
        jax.lax.cond(step >= warm_up, lambda: epsilon, lambda: 1.0)
...
@chex.assert_max_traces(n=1)
def update_policy(
    system_state: DQNSystemState, sampled_batch: DQNBufferData, global_step: int
):
...
while global_step < 200_000:
    ...
    while not done:
        q_values, new_policy_hidden_state = policy_network.apply(
            system_state.network_params.policy_params, obs, policy_hidden_state
        )
    ...
    if episode % 1 == 0:
        print(
            f"EPISODE: {episode}, GLOBAL_STEP: {global_step}, EPISODE_RETURN: {episode_return}, EPSILON: {jnp.round(EPSILON, 2)}, SPS: {sps}"
        )
