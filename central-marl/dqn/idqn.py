...
def dqn_loss(
    ...
    for agent_idx in range(num_agents):
        q_values = jax.vmap(policy_network.apply, in_axes=(None, 0))(
            policy_params, states[:, agent_idx, :]
        )
        target_q_values = jax.vmap(policy_network.apply, in_axes=(None, 0))(
            target_policy_params, next_states[:, agent_idx, :]
        )
