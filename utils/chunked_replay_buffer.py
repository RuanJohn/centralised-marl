...
# TODO: Must definitely handle this in a better way. 
def create_buffer(
    buffer_size: int,
    num_agents: int, 
    num_envs: int,  
    observation_dim: int, 
    # Really represents num_agents in this case. 
    action_dim: int = 1, 
    ...
    buffer_state = BufferState(
        ...
        log_probs = jnp.empty((buffer_size + 1, num_envs, num_agents, action_dim), dtype=jnp.float32), 
        values = jnp.empty((buffer_size + 1, num_envs, num_agents), dtype=jnp.float32), 
        entropy = jnp.empty((buffer_size + 1, num_envs, num_agents, action_dim), dtype=jnp.float32),
        counter = jnp.int32(0), 
        key = buffer_key, 
