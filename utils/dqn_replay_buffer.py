...
def create_buffer(
    ...
    buffer_state = DQNBufferState(
        states=jnp.empty(
            (buffer_size, num_envs, num_agents, observation_dim), dtype=jnp.float32
        ),
        actions=jnp.empty(
            (buffer_size, num_envs, num_agents, action_dim), dtype=jnp.int32
        ),
...
def can_sample(buffer_state: DQNBufferState) -> bool:
